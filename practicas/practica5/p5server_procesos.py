import socket,sys,getopt,time,signal,os
...
class tcp_server():
    def __init__(self,porto):
        try:
            ...
            if len(self.fillos)==0:
                print("[INFO] creando 3 procesos fillos...")
                self.create_fillos()
            while len(self.fillos)>0: #seguimos executando o servidor mentres queden os fillos abertos
                pass
    ...
    def create_fillos(self):
        """metodo para crear os protocolos fillos e poñelos en execucion a espera de clientes"""
        for i in range(3):
            newpid = os.fork()
            if newpid == 0: #bucle para fillos
                while True: #bucle que permite atencion a multiples conexions de clientes                        
                    ...
                    print("Proceso ", os.getpid()) #mostrase o proceso por pantalla
                    ...
                    exit(0) #esta liña fai que se pechen os fillos, se se comenta o servidor segue en execución cos tres fillos dispoñibles para atender os clientes
            else:
                self.fillos.append(newpid)
        print("[INFO] procesos creados")
