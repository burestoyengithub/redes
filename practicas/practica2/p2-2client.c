...
int main(int argc, char * argv[]) {
    ...
    if (argc < 2) {
        ...
        printf("Usar: %s [-i Direccion ip] [-p Numero de porto]\n", argv[0]);
    return (EXIT_FAILURE);
    }
    ...
    while ((opt = getopt(argc, argv, ":i:p:")) != -1) {
        switch (opt) {  
        case 'i':
            ip = optarg; // Argumento direccion ip
            break;
        case 'p':
    ...
    if (ip && porto){
        ...
        // creaccion de la conexion con el servidor
        if (connect(socket_c, (struct sockaddr *)&servaddr, sizeof(servaddr)) != 0) {
            printf("Fallou a conexion co servidor\n");
            return (EXIT_FAILURE);
        } else{
        ...
        // recepcion de datos
        char buf[1024];
        int len = read(socket_c,buf,1024);
        printf("chegaron %d bytes\n",len);
        close(socket_c);
        return (EXIT_SUCCESS);}
}
